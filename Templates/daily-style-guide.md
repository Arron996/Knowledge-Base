...
| 👀 评审中 | 等 CR / 方案评审 | MR 未合入 |
| 🔄 处理中 | 开发进行中 | iDev DOING；可附「下周一发生产」 |
| 🧪 测试中 | 已合测分 / 灰度 | MR/merge 进 **base/测分分支**（如 `develop/base_*`）；口述灰度 % |
| ⏳ 待发生产 | 等发布窗口 | 测分通过、尚未合主分支 |
| ⏳ 问题待解决 | 阻塞 | 会话/blocker |
| ✅ 已发布 | **已合主分支 = 已发生产** | GitLab MR → `main`/`master`；或本地 merge 进主分支 |
| ✅ 已下线 | 下线完成 | 下线类技改 |
| 📝 已完成 | 规划/文档类交付 | H2 规划等 |
...
| 信号 | 正文进度 |
|------|----------|
| MR 合入 `main` / `master` | ✅ **已发生产**（三项目主分支即生产） |
| MR 合入 `develop/base_*` 等测分分支 | 🧪 测试中 |
| 仅有 feature 分支 commit、未合入 | 🔄 处理中 |
| 用户口述灰度比例 | ⏳ 灰度中 X%（写在子项备注，符号仍可用 🧪） |

`collect-gitlab.py` sidecar 字段 `releaseStage`: `production` | `testing` | `unknown` — Agent 写正文时 **production → ✅ 已发生产**。
...
### 待办分栏（今日 vs 明日 vs 待排期）

日报里**三种待办**，禁止混在一栏：

| 区块 | 何时写 | 优先级 | 早晨怎么用 |
|------|--------|--------|------------|
| `## 📋 今日计划` | **9:00 脚本**从上一工作日「明日待办」滚入 | 🔴 必做 · 🟡 重要 · 🟢 可后 | 打开**今日** Daily 按此执行 |
| `## 📋 明日待办` | **19:00 晚间 Agent**填写 | 🔴 必做 · 🟡 重要 · 🟢 可后 | 次日 9:00 滚入「今日计划」 |
| `## 🗂️ 待排期` | 晚间 Agent + 滚动 | 🔵 无日期压力 | 本周有空再扫；**不**当作今日必做 |

**上一工作日**：周一 / 节后首个工作日 9:00，来源为**上周五**或**节前最后工作日**（`workday.py` + `chinese-calendar`），非日历昨天。
